...
public:
  ...
  void computeError()
  {
    ...
    PoseSE2 pose(conf1->pose().x(),conf1->pose().y(),conf1->pose().theta());
    double dist_to_obstacle = robot_model_->calculateDistance(pose, _measurement);
